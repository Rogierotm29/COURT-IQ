...
const GS=()=><style>{`
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}body{background:#07090f}
  ::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-thumb{background:#1e2d40;border-radius:4px}
  @keyframes fadeUp{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}
  @keyframes pulse{0%,100%{opacity:1}50%{opacity:.25}}
  @keyframes spin{to{transform:rotate(360deg)}}
  .fade-up{animation:fadeUp .35s ease both}
  .btn{cursor:pointer;border:none;outline:none;transition:all .15s;font-family:inherit}.btn:hover{filter:brightness(1.15)}
  .card{transition:transform .15s,box-shadow .15s}.card:hover{transform:translateY(-2px);box-shadow:0 8px 28px #00000055}
  input,select{outline:none;font-family:inherit}.spin{animation:spin 1s linear infinite}
`}</style>;
...
const LiveBadge=({live})=><span style={{fontSize:9,fontWeight:700,padding:"2px 8px",borderRadius:20,letterSpacing:.8,background:live?"#00FF9D18":"#1a2535",color:live?"#00FF9D":C.muted}}>{live?"🟢 LIVE API":"📦 Fallback"}</span>;
...
/* ═══════════════════════════════════════════════════════════════════
   FALLBACK DATA (se usa si las APIs fallan)
   ═══════════════════════════════════════════════════════════════════ */
const FB_STANDINGS=[
  {abbr:"OKC",conf:"W",w:55,l:15,streak:"W5",last5:["W","W","W","W","W"]},{abbr:"SAS",conf:"W",w:51,l:18,streak:"W3",last5:["W","W","W","L","W"]},
  {abbr:"LAL",conf:"W",w:44,l:25,streak:"W2",last5:["W","W","L","W","W"]},{abbr:"MIN",conf:"W",w:43,l:27,streak:"W1",last5:["W","L","W","W","L"]},
  {abbr:"DEN",conf:"W",w:42,l:28,streak:"W1",last5:["W","L","W","L","W"]},{abbr:"HOU",conf:"W",w:41,l:27,streak:"L1",last5:["L","W","W","W","L"]},
  {abbr:"PHX",conf:"W",w:39,l:30,streak:"L1",last5:["L","W","L","W","W"]},{abbr:"LAC",conf:"W",w:34,l:35,streak:"L2",last5:["L","L","W","W","L"]},
  {abbr:"POR",conf:"W",w:34,l:36,streak:"W1",last5:["W","L","L","W","W"]},{abbr:"GSW",conf:"W",w:33,l:36,streak:"L1",last5:["L","W","L","W","L"]},
  {abbr:"MEM",conf:"W",w:24,l:44,streak:"L2",last5:["L","L","W","L","L"]},{abbr:"NOP",conf:"W",w:24,l:46,streak:"L1",last5:["L","W","L","L","L"]},
  {abbr:"DAL",conf:"W",w:23,l:47,streak:"L3",last5:["L","L","L","W","L"]},{abbr:"UTA",conf:"W",w:20,l:49,streak:"L2",last5:["L","L","W","L","L"]},
  {abbr:"SAC",conf:"W",w:18,l:52,streak:"L4",last5:["L","L","L","L","W"]},
  {abbr:"DET",conf:"E",w:49,l:19,streak:"W2",last5:["W","W","L","W","W"]},{abbr:"BOS",conf:"E",w:46,l:23,streak:"W1",last5:["W","L","W","W","L"]},
  {abbr:"NYK",conf:"E",w:45,l:25,streak:"W1",last5:["W","L","W","W","L"]},{abbr:"CLE",conf:"E",w:42,l:27,streak:"W2",last5:["W","W","L","W","L"]},
  {abbr:"TOR",conf:"E",w:39,l:29,streak:"L1",last5:["L","W","W","L","W"]},{abbr:"ORL",conf:"E",w:38,l:30,streak:"W1",last5:["W","L","W","W","L"]},
  {abbr:"ATL",conf:"E",w:38,l:31,streak:"L1",last5:["L","W","L","W","W"]},{abbr:"MIA",conf:"E",w:38,l:31,streak:"W1",last5:["W","L","W","L","W"]},
  {abbr:"PHI",conf:"E",w:37,l:32,streak:"L1",last5:["L","W","W","L","W"]},{abbr:"CHA",conf:"E",w:35,l:34,streak:"W1",last5:["W","L","L","W","L"]},
  {abbr:"CHI",conf:"E",w:28,l:41,streak:"L1",last5:["L","W","L","W","L"]},{abbr:"MIL",conf:"E",w:28,l:40,streak:"L2",last5:["L","L","W","L","W"]},
  {abbr:"BKN",conf:"E",w:17,l:52,streak:"L3",last5:["L","L","L","W","L"]},{abbr:"WAS",conf:"E",w:16,l:52,streak:"L4",last5:["L","L","L","L","W"]},
...
/* ═══════════════════════════════════════════════════════════════════
   API FETCHING — llama a /api/... de Vercel
   ═══════════════════════════════════════════════════════════════════ */
...
async function loadScoreboard() {
  try {
    const d = await fetchAPI("/api/scoreboard");
    return d.ok ? d.games : [];
  } catch { return []; }
}
...
async function loadStandings() {
  try {
    const d = await fetchAPI("/api/standings");
    if (!d.ok || !d.standings?.length) return null;
    return d.standings.map(s => ({
      id: s.abbr, ...s, ...tm(s.abbr),
      pct: s.pct || (s.w+s.l>0 ? +(s.w/(s.w+s.l)).toFixed(3) : 0),
      last5: FB_STANDINGS.find(f=>f.abbr===s.abbr)?.last5 || ["W","L","W","W","L"],
      players: ROSTERS[s.abbr] || [],
    }));
  } catch { return null; }
}
...
async function loadPlayers() {
  try {
    const d = await fetchAPI("/api/players");
    if (!d.ok || !d.players?.length) return null;
    return d.players.map((p,i) => ({
      ...p, id: i+1,
      pos: p.pos || "—",
  ...
  return(<div className="fade-up">
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10,marginBottom:28}}>
      ...
      :games.map(g=><Card key={g.id} style={{padding:14}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          {[g.away,g.home].map((t,idx)=><div key={t} style={{textAlign:"center",flex:1}}><div style={{fontSize:18}}>{tm(t).logo}</div><div style={{fontSize:11,color:C.dim,margin:"2px 0"}}>{t}</div><div style={{fontSize:g.status!=="Upcoming"?26:16,fontWeight:900,fontFamily:"'Bebas Neue',sans-serif",color:C.text}}>{g.status!=="Upcoming"?(idx===0?g.awayScore:g.homeScore):"—"}</div></div>)}
        </div></Card>)}
    </div>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}><ST sub="Temporada 2025-26">Top Anotadores</ST><LiveBadge live={live.players}/></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      {[["Este 🌊",east],["Oeste 🌵",west]].map(([label,teams])=><Card key={label}>
        ...
        {teams.slice(0,10).map((t,i)=><div key={t.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderBottom:i<9?`1px solid ${C.border}`:"none"}}>
...
const TeamsTab=({standings,live})=>{
  ...
  return(<div className="fade-up">
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(84px,1fr))",gap:7,marginBottom:22}}>
      {visible.map(t=><button key={t.id} className="btn" onClick={()=>setSel(t)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"10px 6px",borderRadius:12,background:sel?.id===t.id?`${t.color}22`:"#0d1117",border:`1px solid ${sel?.id===t.id?t.color:C.border}`}}><span style={{fontSize:22}}>{t.logo}</span><span style={{fontSize:11,fontWeight:800,color:sel?.id===t.id?t.color:C.dim}}>{t.abbr}</span><span style={{fontSize:9,color:C.muted}}>{t.w}–{t.l}</span></button>)}
    </div>
    {sel&&<><Card style={{marginBottom:14,background:`linear-gradient(135deg,${sel.color}14,${C.card})`,borderColor:`${sel.color}44`}}>
      <div style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:18,flexWrap:"wrap"}}>{[[sel.w,"Wins",C.text],[sel.l,"Losses","#ff6666"],[(sel.pct*100).toFixed(1)+"%","Win%","#00FF9D"],[sel.streak,"Racha",sel.streak?.startsWith("W")?"#00FF9D":"#ff6666"]].map(([v,l,c])=><div key={l} style={{textAlign:"center"}}><div style={{fontSize:26,fontWeight:900,fontFamily:"'Bebas Neue',sans-serif",color:c}}>{v}</div><div style={{fontSize:9,color:C.muted,textTransform:"uppercase",letterSpacing:1}}>{l}</div></div>)}</div>
      </div></Card>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      <Card><div style={{fontSize:10,color:C.muted,textTransform:"uppercase",letterSpacing:2,marginBottom:12}}>Roster 2025-26</div>
        {(sel.players||[]).map((p,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderBottom:i<sel.players.length-1?`1px solid ${C.border}`:"none"}}><div style={{width:22,height:22,borderRadius:"50%",background:`${sel.color}22`,border:`1px solid ${sel.color}44`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:800,color:sel.color,flexShrink:0}}>{i+1}</div><span style={{fontSize:12,fontWeight:600,color:C.text}}>{p}</span></div>)}</Card>
      <Card><div style={{fontSize:10,color:C.muted,textTransform:"uppercase",letterSpacing:2,marginBottom:12}}>Últimos 5 · Info</div>
        ...
        <Divider/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{[["Record",`${sel.w}–${sel.l}`],["Win %",`${(sel.pct*100).toFixed(1)}%`],["Conf.",sel.conf==="E"?"Este":"Oeste"],["División",sel.div]].map(([l,v])=><div key={l} style={{background:"#0a1018",borderRadius:8,padding:"10px"}}><div style={{fontSize:9,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>{l}</div><div style={{fontSize:14,fontWeight:700,color:C.text}}>{v}</div></div>)}</div></Card>
    </div></>}
  </div>);
};

const PlayersTab=({players,live})=>{
  const [sel,setSel]=useState(players[0]);const [search,setSearch]=useState("");const [teamFilter,setTeamFilter]=useState("ALL");
  ...
  const radar=sel?[{s:"PTS",v:Math.min(99,Math.round(pts/38*95))},{s:"AST",v:Math.min(99,Math.round(+(sel.ast||0)/12*95))},{s:"REB",v:Math.min(99,Math.round(+(sel.reb||0)/15*95))},{s:"BLK",v:Math.min(99,Math.round(+(sel.blk||0)/4*95))},{s:"3PT",v:Math.min(99,Math.round(+(sel.fg3Pct||35)/44*95))},{s:"FG%",v:Math.min(99,Math.round(+(sel.fgPct||45)/62*95))}]:[];
  return(<div className="fade-up">
    ...
    <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}>
      <div style={{position:"relative",flex:"1 1 200px"}}><span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:14}}>🔍</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Busca por nombre, equipo o posición..." style={{width:"100%",background:C.card,border:`1px solid ${C.border}`,borderRadius:11,padding:"10px 12px 10px 38px",color:C.text,fontSize:13}}/></div>
    ...
    {sel&&<><Card style={{marginBottom:14,borderLeft:`4px solid ${color}`}}>
      <div style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:12,flexWrap:"wrap"}}>{[["PTS",sel.pts],["AST",sel.ast],["REB",sel.reb],["BLK",sel.blk],["STL",sel.stl],["FG%",sel.fgPct]].map(([l,v])=><div key={l} style={{textAlign:"center"}}><div style={{fontSize:22,fontWeight:900,fontFamily:"'Bebas Neue',sans-serif",color:C.text}}>{v}</div><div style={{fontSize:9,color:C.muted,textTransform:"uppercase",letterSpacing:1}}>{l}</div></div>)}</div>
      </div></Card>
    <div style={{display:"grid",gridTemplateColumns:"1fr 260px",gap:14,marginBottom:14}}>
      <Card><div style={{fontSize:10,color:C.muted,textTransform:"uppercase",letterSpacing:2,marginBottom:10}}>Tendencia de Puntos</div>
        <ResponsiveContainer width="100%" height={165}><AreaChart data={trend}><defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={color} stopOpacity={.3}/><stop offset="95%" stopColor={color} stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke={C.border}/><XAxis dataKey="g" tick={{fill:C.muted,fontSize:9}} axisLine={false} tickLine={false}/><YAxis tick={{fill:C.muted,fontSize:9}} axisLine={false} tickLine={false}/><Tooltip content={<TT/>}/><Area type="monotone" dataKey="pts" name="PTS" stroke={color} strokeWidth={2.5} fill="url(#ag)" dot={false} activeDot={{r:5}}/></AreaChart></ResponsiveContainer></Card>
...
const PickemTab=({games})=>{const [picks,setPicks]=useState({});const [submitted,setSubmitted]=useState(false);const upcoming=games.filter(g=>g.status==="Upcoming");const done=upcoming.filter(g=>picks[g.id]).length;
  return(<div className="fade-up"><ST sub="Compite con amigos">Pick'em Daily 🎯</ST>
    ...
    :upcoming.map(g=><Card key={g.id} style={{marginBottom:10}}><div style={{fontSize:9,color:C.muted,marginBottom:10,textTransform:"uppercase",letterSpacing:1.5}}>⏰ Elige al ganador</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:8,alignItems:"center"}}>{[g.away,null,g.home].map((item,idx)=>idx===1?<div key="vs" style={{fontSize:11,color:C.muted,fontWeight:700,textAlign:"center"}}>VS</div>
      :<button key={item} className="btn" onClick={()=>!submitted&&setPicks(p=>({...p,[g.id]:item}))} style={{padding:"12px",borderRadius:11,textAlign:"center",background:picks[g.id]===item?`${tm(item).color}22`:C.card,border:`2px solid ${picks[g.id]===item?tm(item).color:C.border}`,color:picks[g.id]===item?tm(item).color:C.text,fontSize:16,fontWeight:900,fontFamily:"'Bebas Neue',sans-serif",width:"100%"}}>{tm(item).logo} {picks[g.id]===item&&"✓ "}{item}</button>)}</div></Card>)}
...
export default function App(){
  ...
  const [players,setPlayers]=useState(FB_PLAYERS);
  ...
  const refreshAll=useCallback(async()=>{
    ...
    if(pl&&pl.length>=10){setPlayers(pl);setLive(l=>({...l,players:true}));}
    setLastUpd(new Date());
    setLoading(false);
  },[]);
    ...
    <div style={{background:"#0a0f17ee",borderBottom:`1px solid ${C.border}`,padding:"11px 22px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,backdropFilter:"blur(16px)"}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <div><div style={{fontSize:15,fontWeight:900,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:1,lineHeight:1}}>COURT IQ</div>
          <div style={{fontSize:8,color:C.muted,letterSpacing:2,textTransform:"uppercase"}}>{lastUpd?`Live · ${lastUpd.toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit"})}`:"NBA 2025-26"}</div></div>
      </div>
